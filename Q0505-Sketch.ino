/*
     ...
     SKETCH:  	Módulo Sensor Biométrico
     DATA:    	05/11/2020
*/
...
bool grava = false; // TRUE PARA GRAVAR UMA IMPRESSÃO NO ID = 1 DURANTE SETUP
int id = 1;
...
void loop() {
  ...
  if (leitura == 1) {
    ...
    delay(1000);
    digitalWrite(pinRele, LOW);
  } else if ( leitura == FINGERPRINT_FEATUREFAIL ||
              leitura == FINGERPRINT_INVALIDIMAGE ||
              leitura == FINGERPRINT_INVALIDIMAGE ||
              leitura == FINGERPRINT_NOTFOUND) {
                Serial.print("ERROU");
